...
public:
    ...
    bool parse_from_file(const std::string& file_path) {
        ...
        // 更新库级别属性（如果还没有设置）
        if (cell_library_.get_library_name().empty() && !lib.get_library_name().empty()) {
...
public:
    ...
    bool parse_from_file(const std::string& file_path) {
        ...
        celllib::LibertyParser parser(file_path);
        if (!parser.is_valid()) {
            std::cerr << "Parser initialization error: " << parser.get_error() << std::endl;
            return false;
        }
    ...
    void print_parsed_content() const {
        ...
        std::cout << "Capacitance Unit: " << cell_library_.get_capacitance_unit() << " ff\n";
        ...
        if (!template_names.empty()) {
            ...
            for (const auto& tname : template_names) {
                ...
                if (templ) {
                    ...
                    std::cout << " [index_1: " << templ->index_1.size() 
                              << " values";
                    if (!templ->index_1.empty()) {
                        std::cout << " (";
                        for (size_t i = 0; i < std::min(templ->index_1.size(), size_t(3)); ++i) {
                            if (i > 0) std::cout << ", ";
                            std::cout << templ->index_1[i];
                        }
                        if (templ->index_1.size() > 3) std::cout << "...";
                        std::cout << ")";
                    }
                    std::cout << ", index_2: " << templ->index_2.size() << " values";
                    if (!templ->index_2.empty()) {
                        std::cout << " (";
                        for (size_t i = 0; i < std::min(templ->index_2.size(), size_t(3)); ++i) {
                            if (i > 0) std::cout << ", ";
                            std::cout << templ->index_2[i];
                        }
                        if (templ->index_2.size() > 3) std::cout << "...";
                        std::cout << ")";
                    }
                    std::cout << "]\n";
                }
        ...
        for (const auto& cell_name : cell_library_.get_cell_names()) {
            ...
            if (cell->cell_leakage_power.has_value()) {
                std::cout << "  Cell Leakage Power: " << cell->cell_leakage_power.value() << "\n";
            }
                ...
                for (const auto& pin_name : input_pins) {
                    ...
                    if (pin) {
                        ...
                        if (pin->capacitance.has_value()) {
                            std::cout << " (capacitance: " << pin->capacitance.value() << " ff)";
                        }
                        if (pin->rise_capacitance.has_value()) {
                            std::cout << ", rise_cap: " << pin->rise_capacitance.value() << " ff";
                        }
                        if (pin->fall_capacitance.has_value()) {
                            std::cout << ", fall_cap: " << pin->fall_capacitance.value() << " ff";
                        }
                        if (pin->max_capacitance.has_value()) {
                            std::cout << ", max_cap: " << pin->max_capacitance.value() << " ff";
                        }
                        if (pin->is_clock) {
                            std::cout << " [CLOCK]";
                        }
                        ...
                        if (!pin->timing_arcs.empty()) {
                            ...
                            for (size_t i = 0; i < pin->timing_arcs.size(); ++i) {
                                ...
                                switch (arc.timing_type) {
                                    case celllib::TimingType::COMBINATIONAL: timing_type_str = "combinational"; break;
                                    case celllib::TimingType::SETUP_RISING: timing_type_str = "setup_rising"; break;
                                    case celllib::TimingType::SETUP_FALLING: timing_type_str = "setup_falling"; break;
                                    case celllib::TimingType::HOLD_RISING: timing_type_str = "hold_rising"; break;
                                    case celllib::TimingType::HOLD_FALLING: timing_type_str = "hold_falling"; break;
                                    case celllib::TimingType::RISING_EDGE: timing_type_str = "rising_edge"; break;
                                    case celllib::TimingType::FALLING_EDGE: timing_type_str = "falling_edge"; break;
                                    case celllib::TimingType::MIN_PULSE_WIDTH: timing_type_str = "min_pulse_width"; break;
                                    default: timing_type_str = "unknown"; break;
                                }
                ...
                for (const auto& pin_name : output_pins) {
                    ...
                    if (pin) {
                        ...
                        if (!pin->timing_arcs.empty()) {
                            ...
                            for (size_t i = 0; i < pin->timing_arcs.size(); ++i) {
                                ...
                                if (arc.cell_rise.has_value()) {
                                    ...
                                    std::cout << ", cell_rise LUT (" << lut.index_1.size() 
                                              << "x" << lut.index_2.size() << ")";
                                    if (lut.template_name.has_value()) {
                                        std::cout << " [template: " << lut.template_name.value() << "]";
                                    }
                                ...
                                if (arc.cell_fall.has_value()) {
                                    ...
                                    std::cout << ", cell_fall LUT (" << lut.index_1.size() 
                                              << "x" << lut.index_2.size() << ")";
                                    if (lut.template_name.has_value()) {
                                        std::cout << " [template: " << lut.template_name.value() << "]";
                                    }
                        ...
                        if (!pin->internal_power.empty()) {
                            std::cout << "      Internal Power Entries (" << pin->internal_power.size() << "):\n";
                            for (size_t j = 0; j < pin->internal_power.size(); ++j) {
                                ...
                                if (ip.rise_power.has_value()) {
                                    ...
                                    std::cout << ", rise_power LUT (" << lut.index_1.size() 
                                              << (lut.index_2.empty() ? "" : "x" + std::to_string(lut.index_2.size())) << ")";
                                    if (lut.template_name.has_value()) {
                                        std::cout << " [template: " << lut.template_name.value() << "]";
                                    }
                                }
