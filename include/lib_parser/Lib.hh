...
#include "ista_compat.hpp"
#include "ista_types.hpp"
...
class LibCell : public LibObject
{
 public:
  ...
  auto& get_cell_port_buses() { return _cell_port_buses; }
...
class LibLibrary : public LibObject
{
 public:
  ...
  /** Return all lut template names (for converter iteration). */
  std::vector<std::string> get_lut_template_names() const
  {
    std::vector<std::string> r;
    for (const auto& p : _str2template) r.push_back(p.first);
    return r;
  }
