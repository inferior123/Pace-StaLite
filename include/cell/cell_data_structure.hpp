 ...
 */
enum class PinDirection { INPUT, OUTPUT, INOUT, INTERNAL };
...
struct LookupTable {
  ...
  std::optional<std::string>
      template_name; // 引用的table_template名称（如 "Timing_7_7", "Hold_3_3"）
...
struct TimingArc {
  ...
  bool is_rising_triggered() { return timing_type == TimingType::RISING_EDGE; }
};
...
struct TableTemplate {
  ...
  explicit TableTemplate(const std::string &template_name)
      : name(template_name) {}
};
