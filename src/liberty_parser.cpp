...
PinDirection LibertyParser::parse_pin_direction(const std::string& dir_str) {
    if (dir_str == "input") return PinDirection::INPUT;
    if (dir_str == "output") return PinDirection::OUTPUT;
    if (dir_str == "inout") return PinDirection::INOUT;
    if (dir_str == "internal") return PinDirection::INTERNAL;
    return PinDirection::INPUT;
}

TimingSense LibertyParser::parse_timing_sense(const std::string& sense_str) {
    if (sense_str == "positive_unate") return TimingSense::POSITIVE_UNATE;
    if (sense_str == "negative_unate") return TimingSense::NEGATIVE_UNATE;
    if (sense_str == "non_unate") return TimingSense::NON_UNATE;
    return TimingSense::POSITIVE_UNATE;
}

TimingType LibertyParser::parse_timing_type(const std::string& type_str) {
    if (type_str == "combinational") return TimingType::COMBINATIONAL;
    if (type_str == "setup_rising") return TimingType::SETUP_RISING;
    if (type_str == "setup_falling") return TimingType::SETUP_FALLING;
    if (type_str == "hold_rising") return TimingType::HOLD_RISING;
    if (type_str == "hold_falling") return TimingType::HOLD_FALLING;
    if (type_str == "rising_edge") return TimingType::RISING_EDGE;
    if (type_str == "falling_edge") return TimingType::FALLING_EDGE;
    if (type_str == "clear") return TimingType::CLEAR;
    if (type_str == "preset") return TimingType::PRESET;
    if (type_str == "min_pulse_width") return TimingType::MIN_PULSE_WIDTH;
...
void LibertyParser::parse_library() {
    ...
    while (!eof() && !has_error) {
        ...
        // std::cerr << "Debug: Read token: " << token << " at line " << line_number << std::endl;
        ...
        if (token == "cell") {
            ...
            if (!has_error && !cell.name.empty()) {
                ...
                if (has_error) {
                    std::cerr << "Error parsing cell at line " << line_number << ", error: " << error_message << std::endl;
                }
                if (cell.name.empty()) {
                    std::cerr << "Warning: Cell name is empty after parsing at line " << line_number << std::endl;
                }
            }
    ...
    if (cell_count == 0 && !has_error) {
        std::cerr << "Warning: No cells were parsed from library. Total tokens processed." << std::endl;
    }
}
...
void LibertyParser::parse_table_template() {
    ...
    // 调试输出
    std::cout << "  [DEBUG] Parsed table template: " << template_name 
              << " (var1=" << (templ.variable_1.has_value() ? templ.variable_1.value() : "none")
              << ", var2=" << (templ.variable_2.has_value() ? templ.variable_2.value() : "none")
...
void LibertyParser::parse_pin_attribute(Pin& pin, const std::string& attr) {
    ...
    if (peek_char() == ':') {
        ...
        if (attr == "direction") {
            ...
            pin.rise_capacitance = read_number();
        } else if (attr == "fall_capacitance") {
            pin.fall_capacitance = read_number();
...
void LibertyParser::parse_timing_arc(TimingArc& arc, const std::string& output_pin_name) {
    while (!eof() && !has_error) {
        skip_whitespace();
        skip_comment();
...
void LibertyParser::parse_timing_arc_lookup_table(TimingArc& arc, const std::string& attr) {
    skip_whitespace();
    std::string template_name;
...
void LibertyParser::parse_timing_arc_constraint(TimingArc& arc, const std::string& attr) {
    skip_whitespace();
    std::string template_name;
...
void LibertyParser::parse_timing_arc_attribute(TimingArc& arc, const std::string& attr) {
    skip_whitespace();
...
void LibertyParser::parse_lookup_table(LookupTable& lut, const std::string& table_type) {
    while (!eof() && !has_error) {
        skip_whitespace();
        skip_comment();
    ...
    // 对于timing相关的查找表（cell_rise, cell_fall, rise_transition, fall_transition, 
    // fall_constraint, rise_constraint），index_1是时间值，需要转换
...
void LibertyParser::parse_values_list(std::vector<std::vector<double>>& values, size_t num_rows) {
    skip_whitespace();
...
void LibertyParser::parse_comma_separated_numbers(const std::string& str, std::vector<double>& numbers) {
...
void LibertyParser::parse_unquoted_value_row(std::vector<std::vector<double>>& values) {
    double value = read_number();
