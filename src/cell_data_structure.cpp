...
std::optional<double> LookupTable::lookup_nearest(double idx1, double idx2) const {
    if (index_1.empty() || index_2.empty() || values.empty()) {
        return std::nullopt;
    }
    
    // 找到最近的索引
    auto find_nearest_index = [](const std::vector<double>& indices, double value) -> size_t {
...
void StandardCell::add_pin(const Pin& pin) {
    pins[pin.name] = pin;
}
...
const TableTemplate* CellLibrary::get_table_template(const std::string& name) const {
...
inline double insert_caculate(double x0, double x1, double x2, double y0, double y1, double y2, double T11, double T12, double T21, double T22) {
...
double CellLibrary::caculate_lookuptable(const LookupTable& tb, double x0, double y0, const std::string &va_name_1, const std::string &va_name_2) const {
    if(!tb.template_name.has_value()) {
        std::cerr << "no spceficed template for table " << std::endl;
        assert(false);
    }

    if(tb.index_1.empty() || tb.index_2.empty() || tb.values.empty()) {
        std::cerr << "empty lookup table" << std::endl;
        assert(false);
    }

    const TableTemplate* templ = get_table_template(tb.template_name.value());
    if(!templ) {
        std::cerr << "template not found: " << tb.template_name.value() << std::endl;
        assert(false);
    }

    // Determine which variable maps to which index
    // x0 and y0 are already provided as parameters, we just need to map them correctly
    ...
    if(templ->variable_1.has_value() && templ->variable_2.has_value()) {
        if(templ->variable_1.value() == va_name_1 && templ->variable_2.value() == va_name_2) {
            // variable_1 -> index_1, variable_2 -> index_2
            // x0 corresponds to va_name_1 (variable_1), y0 corresponds to va_name_2 (variable_2)
            mapped_x0 = x0;
            mapped_y0 = y0;
            x_indices = &tb.index_1;
            y_indices = &tb.index_2;
        } else if(templ->variable_2.value() == va_name_1 && templ->variable_1.value() == va_name_2) {
            // variable_2 -> index_2, variable_1 -> index_1 (swapped)
            // x0 corresponds to va_name_1 (variable_2), which maps to index_2
            // y0 corresponds to va_name_2 (variable_1), which maps to index_1
            // So we need to swap the mapped values: mapped_x0 gets y0, mapped_y0 gets x0
            mapped_x0 = y0;  // y0 maps to variable_1 (index_1), so use it for x_indices
            mapped_y0 = x0;  // x0 maps to variable_2 (index_2), so use it for y_indices
            x_indices = &tb.index_1;  // Keep x_indices pointing to index_1
            y_indices = &tb.index_2;  // Keep y_indices pointing to index_2
        } else {
            std::cerr << "mismatch index name for caculate lookuptable" << std::endl;
            std::cerr << "  templ: " << templ->variable_1.value() << " " << templ->variable_2.value() << std::endl;
    ...
    if(x_idx1 >= tb.values.size() || x_idx2 >= tb.values.size() ||
       y_idx1 >= tb.values[x_idx1].size() || y_idx2 >= tb.values[x_idx1].size() ||
       y_idx1 >= tb.values[x_idx2].size() || y_idx2 >= tb.values[x_idx2].size()) {
    ...
    // std::cout << "      Input: x0=" << x0 << " (" << va_name_1 << "), y0=" << y0 << " (" << va_name_2 << ")" << std::endl;
    // std::cout << "      Mapped: mapped_x0=" << mapped_x0 << ", mapped_y0=" << mapped_y0 << std::endl;
    // std::cout << "      X indices: [" << x1 << ", " << x2 << "] (indices: " << x_idx1 << ", " << x_idx2 << ")" << std::endl;
    // std::cout << "      Y indices: [" << y1 << ", " << y2 << "] (indices: " << y_idx1 << ", " << y_idx2 << ")" << std::endl;
    // std::cout << "      Corner values: T11=" << T11 << ", T12=" << T12 << ", T21=" << T21 << ", T22=" << T22 << std::endl;
    
    double result = insert_caculate(mapped_x0, x1, x2, mapped_y0, y1, y2, T11, T12, T21, T22);
    ...
    // std::cout << "      Interpolation weights: x01=" << x01 << ", x20=" << x20 << ", y01=" << y01 << ", y20=" << y20 << std::endl;
