...
#include "lib_parser/LibParserRustC.hh"
...
unsigned RustLibertyReader::visitSimpleAttri(RustLibertySimpleAttrStmt* attri) {
  ...
  // 与 simple_nangate 一致：capacitance 保持 ff，LUT index (total_output_net_capacitance) 也为 ff
  double cap_unit_convert = 1.0;
  ...
  std::map<std::string, std::function<void()>> process_attri = {
      ...
      {"clock",
       [=]() {
         if (lib_port) {
           auto* rust_attri_value = rust_convert_string_value(attri_value);
           bool is_clock = (Str::noCaseEqual(rust_attri_value->value, "true"));
           lib_port->set_clock_gate_clock_pin(is_clock);  // isClock() 用此判断
           rust_free_string_value(rust_attri_value);
         }
       }},
  ...
  if (process_attri.find(attri_name) != process_attri.end()) {
...
unsigned RustLibertyReader::visitPin(RustLibertyGroupStmt* group) {
  ...
  if (ret_val.empty()) {
    ...
    for (int index = port_range_left; index >= port_range_right; --index) {
      std::string one_port_name = Str::printf("%s[%d]", port_bus_name.c_str(), index);
...
unsigned RustLibertyReader::visitGroup(RustLibertyGroupStmt* group) {
  ...
  static const ista::BTreeSet<std::string> table_names = {
      "cell_rise",       "cell_fall",       "rise_transition",
      "fall_transition", "rise_constraint", "fall_constraint"};
  static const ista::BTreeSet<std::string> power_table_names = {"rise_power",
                                                                "fall_power"};
  ...
  if (visit_fun_map.find(group_name) != visit_fun_map.end()) {
    auto read_func = visit_fun_map[group_name];
    is_ok = read_func(group);
  } else if (table_names.find(group_name) != table_names.end()) {
    is_ok = visitTable(group);
  } else if (power_table_names.find(group_name) != power_table_names.end()) {
